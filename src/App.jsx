      ...
      <div style={{marginBottom:"1.25rem"}}>
        ...
        {showForm && (
          <div style={{background:"#E8ECF0",boxShadow:"6px 6px 12px #C5C9D4, -6px -6px 12px #FFFFFF",borderRadius:16,padding:"16px",marginTop:12}}>
            ...
            <div style={row}><span style={lbl}>機種名</span><div style={{flex:1}}><input value={fMachine} onChange={e=>{setFMachine(e.target.value);setMachineSuggestion(null);}} onBlur={e=>checkMachineName(e.target.value)} placeholder="例: バジリスク絆2（任意）" list="machine-candidates" style={{...inp,width:"100%",marginBottom:machineSuggestion?4:0}}/>
    ...
    try {
      const res = await fetch("/api/claude", {
        ...
        body: JSON.stringify({
          ...
          system: 'パチスロライブラリの収集アシスタントです。実在する機種名・具体的な情報のみ使用。JSON形式のみで返答: {"cat":"bonus|spec|quote|memory","source":"manual","machine":"機種名","title":"30文字以内","body":"20〜40文字。リンクを押したくなる一言。「え、そうなの？」「知らなかった！」と思わせる煽り気味の短文。詳細はリンク先へ誘導するイメージ。","quality":3,"dupKey":"機種名_キー","eng":{}}',
...
・bodyは20〜40文字の一言で終わらせること。「え、そうなの？」「知らなかった！」と読んだ人がリンクを押したくなる煽り気味の短文にすること。説明しすぎない。` + badContext;
