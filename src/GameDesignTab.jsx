...
// ゲーム性データ（gameDesignLibrary.jsonの内容をカテゴリ別に整理）
const DATA = {
  CZ: {
    types: {
      "軽量CZ型": {
  ...
  ボーナス: {
    types: {
      ...
      "ST（ストック）型": {
        ...
        emotion: null,
        machines: [
          { name: "ミリオンゴッド（初代）", detail: GAME_LIBRARY.classicMachines ? null : null },
        ],
      },
    },
  },
...
export default function GameDesignTab() {
  ...
  const selType  = activeType && types[activeType] ? types[activeType] : null;
  ...
  async function submitCorrection(machine) {
    ...
    try {
      await fetch("https://vpzbtuucopucablwyqeq.supabase.co/rest/v1/collection_requests", {
        ...
        body: JSON.stringify({
          theme: `修正提案：${activeCat}・${activeType}・${machine}\n内容：${corrText.trim()}`,
          status: "pending",
        }),
      });
      setCorrSent(true);
  ...
  return (
    <div>
        ...
        {Object.keys(CAT_CONFIG).map(cat => {
          ...
          return (
            <button key={cat} onClick={() => switchCat(cat)}
              style={{ flexShrink: 0, padding: "7px 14px", border: "none", borderRadius: 10, fontSize: 13,
                fontWeight: on ? 700 : 500, background: on ? "#E0E4E8" : "#E8ECF0",
                color: on ? CAT_CONFIG[cat].accent : "#888", cursor: "pointer",
                boxShadow: on ? "inset 3px 3px 6px #C5C9D4, inset -2px -2px 5px #FFFFFF" : "2px 2px 5px #C5C9D4, -2px -2px 5px #FFFFFF",
      ...
      {/* 準備中カテゴリ */}
      {catData._note && (
        <div style={{ background: "#E8ECF0", boxShadow: "inset 3px 3px 6px #C5C9D4, inset -3px -3px 6px #FFFFFF",
          borderRadius: 12, padding: 16, textAlign: "center", color: "#888", fontSize: 14, marginBottom: 12 }}>
          {catData._note}
          <div style={{ marginTop: 10 }}>
            <span style={{ fontSize: 12, color: "#aaa" }}>情報提供は下の「修正・追加」ボタンから送れます</span>
          </div>
        </div>
      )}
          ...
          {typeKeys.map(t => {
            ...
            return (
              <button key={t} onClick={() => switchType(t)}
                style={{ padding: "8px 14px", border: "none", borderRadius: 20, fontSize: 13,
                  fontWeight: on ? 700 : 500, background: on ? catCfg.bg : "#E8ECF0",
                  color: on ? catCfg.accent : "#555", cursor: "pointer",
                  boxShadow: on ? "inset 2px 2px 5px rgba(0,0,0,0.12)" : "2px 2px 5px #C5C9D4, -2px -2px 5px #FFFFFF",
      ...
      {/* 型の詳細パネル */}
      {selType && (
        <div style={{ background: "#E8ECF0", boxShadow: "inset 4px 4px 8px #C5C9D4, inset -4px -4px 8px #FFFFFF",
          borderRadius: 14, padding: 14, marginBottom: 12 }}>

          {/* 型の説明 */}
          <div style={{ fontSize: 11, color: "#aaa", fontWeight: 500, marginBottom: 4 }}>型の概要</div>
          <div style={{ fontSize: 13, color: "#444", lineHeight: 1.65, marginBottom: 6 }}>
            {selType.description}
          </div>
          {selType.emotion && (
            <div style={{ fontSize: 12, color: catCfg.accent, fontStyle: "italic", marginBottom: 4 }}>
              💭 {selType.emotion}
            </div>
          )}
          {selType.note && (
            <div style={{ fontSize: 12, color: "#888", padding: "6px 10px", background: "rgba(0,0,0,0.04)",
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 11, color: "#aaa", fontWeight: 500, marginBottom: 8 }}>この型の機種</div>

            {(selType.machines || []).length === 0 && (
              <div style={{ fontSize: 13, color: "#aaa", textAlign: "center", padding: 12 }}>
                機種データなし（情報提供募集中）
              </div>
            )}

            {(selType.machines || []).map((m, i) => {
              const featureName = FEATURE_NAMES[m.name]?.[activeCat] ?? null;
              const isOpen      = openIdx === i;
              const mKey        = `${activeCat}__${activeType}__${i}`;
              const isCorrecting = corrKey === mKey;

              return (
                <div key={i} style={{ marginBottom: 8, borderRadius: 12, overflow: "hidden",
                  background: "#EAEEF2", boxShadow: "2px 2px 5px #C5C9D4,-2px -2px 5px #FFFFFF" }}>

                  {/* 機種ヘッダー行 */}
                  <button onClick={() => { setOpenIdx(isOpen ? null : i); setCorrKey(null); setCorrSent(false); }}
                    style={{ width: "100%", display: "flex", alignItems: "center", gap: 8,
                      padding: "10px 12px", border: "none", background: "none", cursor: "pointer", textAlign: "left" }}>
                    <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: "#333" }}>{m.name}</span>
                    {featureName && (
                      <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 10,
                        background: catCfg.bg, color: catCfg.accent, fontWeight: 600, flexShrink: 0,
                    ...
                    <div style={{ padding: "0 12px 12px" }}>
                      ...
                      {!isCorrecting && (
                        <button onClick={() => { setCorrKey(mKey); setCorrSent(false); setCorrText(""); }}
                          style={{ fontSize: 11, color: "#888", background: "none",
                            border: "1px solid #ccc", borderRadius: 8, padding: "4px 10px", cursor: "pointer" }}>
                          ✏️ 情報を修正・追加する
                        </button>
                      )}
                          ...
                          {corrSent ? (
                            ...
                            <>
                              <textarea
                                ...
                                style={{ width: "100%", minHeight: 80, fontSize: 13, borderRadius: 8,
                              ...
                              <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                                <button onClick={() => submitCorrection(m.name)}
                                  ...
                                  style={{ fontSize: 12, padding: "6px 14px", border: "none", borderRadius: 8,
                                    ...
                                    color: "#fff", cursor: corrLoading || !corrText.trim() ? "default" : "pointer",
                                    fontWeight: 600 }}>
                                  {corrLoading ? "送信中..." : "送信"}
                                </button>
                                <button onClick={() => { setCorrKey(null); setCorrText(""); }}
                                  style={{ fontSize: 12, padding: "6px 12px", border: "1px solid #ccc",
