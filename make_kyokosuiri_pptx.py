"""
L虚構推理 機種説明＋分析 統合資料  （ディライト（D-LIGHT）・2026年4月6日導入）
出力: proposals/機種分析/虚構推理/kyokosuiri_guide_v1.pptx
...
OUT_PATH = os.path.join(os.path.dirname(__file__),
           "proposals", "機種分析", "虚構推理", "kyokosuiri_guide_v1.pptx")
...
def s_cz(prs):
    ...
    tb(s, lx, ep_y + ep_h + Emu(40000), lw, Emu(170000),
       "▲ EP3失敗→次回CZに持越し（キャリーオーバー）　EP4・5は次回から継続", 7,
    ...
    tb(s, lx + Emu(75000), ko_y + Emu(310000), lw - Emu(100000), ko_h - Emu(370000),
       ...
       "・5エピソードを全部突破するとCZ成功→ボーナス確定\n"
       "・「今回3エピソード突破→次回は残り2エピソードのみ」\n"
       "・失敗が「無」ではなく「前進」になる唯一無二の設計\n"
       "・「あと○エピソード」という粘る動機を常に生み続ける",
       8, color=C_WHITE)
    ...
    tb(s, rx + Emu(75000), ry + Emu(305000), rw - Emu(100000), Emu(340000),
       "筐体上部に搭載された専用デバイス。最終G（判定ゲーム）にARROWの色が変化し\nCZ成否・期待度を告知する。",
       8, color=C_WHITE)

    # ARROW色変化カラーボックス
    arrow_y = ry + Emu(740000)
    tb(s, rx, arrow_y - Emu(60000), rw, Emu(200000),
       "ARROW色変化（白 → 赤 → 紫 と変わるほど期待度UP）", 7.5, bold=True, color=C_CYAN)

    arrow_colors = [
        (RGBColor(0xE0, 0xE0, 0xE0), "白",    "低期待度"),
        (C_BLUE,                      "青",    "やや期待"),
        (C_YEL,                       "黄",    "チャンス"),
        (C_GREEN,                     "緑",    "高期待"),
        (C_RED,                       "赤",    "激アツ"),
    ...
    tb(s, rx + Emu(75000), exp_y + Emu(310000), rw - Emu(100000), exp_h - Emu(370000),
       "・最終Gに全員の視線がARROWに集まる「1点集中の緊張感」\n"
       "・視野の端でARROWの色変化を察知する体験\n"
       "・画面外デバイスが光るという「非日常感」\n"
       "・「ARROWが赤く光った瞬間に声が出た」という体験報告が続出",
       8, color=C_WHITE)

    footer(s,
           "設計コメント：CZはエピソード突破型で失敗が積み上がり、ARROW告知が最終Gに緊張の頂点を作る二重構造",
           "補足：ARROWの色は白→青→黄→緑→赤→紫点滅→赤点滅の順に期待度上昇。赤以上は激アツ判定")
    net_note(s)
...
def s_kykoren(prs):
    ...
    footer(s,
           "設計コメント：Long率が高設定の最重要判別指標。Long突入時は9400枚超の爆発力を秘める",
           "補足：虚構連中もARROW告知が機能するため、ループするたびに再度の緊張と興奮体験が続く設計")
    net_note(s)
...
def s_design(prs):
    ...
    footer(s,
           "設計コメント：「失敗が前進になる」設計と「外部デバイス告知」は、他のパチスロ機種に前例のない組み合わせ",
           "補足：この2つの要素が組み合わさることで「積み上げの達成感」と「ARROW確認の緊張感」が相互強化される")
    net_note(s)
...
def s_pros_cons(prs):
    ...
    pros = [
        (C_GREEN, "エピソードキャリーオーバー",
         "失敗が前進になる唯一無二の設計。\nパチスロ史上でも稀なCZ設計思想。"),
        (C_CYAN,  "ARROW告知デバイス",
         "外部デバイスによる告知体験は他機種にない独自性。\n話題性・ホール内コミュニティ形成に貢献。"),
        (C_PUR2,  "虚構連モード3種の明確化",
         "Short/Middle/Longという上位状態の可視化が\nプレイヤーの目標設定を明確にする。"),
        (C_GOLD,  "高設定時の爆発力",
...
def s_matome(prs):
    ...
    elems = [
        ...
        (C_CYAN,  "② 外部告知デバイスARROWの体験価値",
         "画面外のデバイスが光るという非日常体験。\n"
         "話題性・ホール内コミュニティ形成を同時に実現。\n"
         "「みんなで見守る」設計がホール体験を変える。"),
        (C_GREEN, "③ 3種モードによる上位状態の可視化",
         "Short/Middle/Longという明確な上位状態分類が\n"
         "「今自分がどのモードにいるか」を意識させる。\n"
         "目標の可視化が来店継続・稼働継続に繋がる。"),
    ]
    ...
    principles = [
        (C_PUR,   "失敗をストックする設計は「諦めない」心理を生む"),
        (C_CYAN,  "外部デバイス活用で画面内演出の限界を突破できる"),
        (C_PUR2,  "推理×物語の融合が知的楽しさをゲーム性に加える"),
        (C_GOLD,  "上位状態の種別可視化がプレイヤーの目標を明確化"),
