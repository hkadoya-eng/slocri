...
取れないときは「測定不能」として残りの軸で再配分する。
  ...
  ② 持続      N週目の総稼働 ÷ 2週目の総稼働   ← 起点を2週目にして初週のご祝儀スパイクを避ける
...
ウェイト: ①35 ②20 ③20 ④15 ⑤10（実測で決定・下記参照）。測定不能の軸は分母から外して残りへ再配分。
...
#   ③候補の増分: 総稼働+0.028／台数比+0.021／初週台数+0.018／相対粗利+0.002／出玉率±0.000
#   ②候補の増分(①③の上): 8週÷2週の総稼働+0.018／8週÷4週+0.014／4週÷初週の稼働値+0.001
#   競合（立ち上がり8週に入ってきた新台数・その総稼働）は r=-0.12/-0.10・増分+0.001＝ほぼ無関係
#   ①②③は相互相関0.69〜0.96で独立な3次元ではない。生死の予測は①でほぼ足りる。
#   → 5軸スコアは予測モデルではなく「どういう台か」の記述。重みは増分の実測に合わせる。
WEIGHTS = {"demand": 35, "retention": 20, "hall": 20, "interest": 15, "satisfaction": 10}
...
def axes_at(ser, n):
    ...
    # 総稼働の維持率。窓を2週目→N週目に取る理由:
    #  ・初週は導入時のご祝儀で跳ねるのでノイズが大きい
    #  ・窓が6週間ぶんあるので減衰の差が出る（旧定義の初週→4週は3週間しかなかった）
    #  ・総稼働ベースなので台数の減りも織り込める
    # 実測: ①+これ+③ で R²=0.487（旧定義の4週目÷初週の稼働値だと0.470）
    if len(ser) >= n and a["hall"] is not None:
        tv2 = ser[1]["k"] / 100 * ser[1]["u"] if ser[1]["u"] else None
        if tv2:
            a["retention"] = a["hall"] / tv2 * 100
    return a
...
def main():
    ...
    a["interest"] = interest
    a["satisfaction"] = dmm
    LABEL = {"demand": "① 需要（%d週目の稼働値）" % n, "retention": "② 持続（%d週目÷2週目の総稼働）" % n,
             "hall": "③ 総稼働（%d週目・平均台換算）" % n,
             "interest": "④ 関心度（YouTube上位20本の累計）", "satisfaction": "⑤ 納得感（DMM評価点）"}
    UNIT = {"demand": "%.0f%%", "retention": "%.0f%%", "hall": "%.2f台分",
