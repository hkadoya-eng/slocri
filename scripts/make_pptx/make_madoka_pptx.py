...
def net_note(slide, text, design_comment):
    ...
    bar.fill.solid()
    bar.fill.fore_color.rgb = RGBColor(0x22, 0x10, 0x30)
    bar.line.fill.background()
...
def slide1_title(prs):
    ...
    y = 1.18
    for k, v in specs:
        tb(slide, f"◆ {k}：", 0.35, y, 1.55, 0.28, font_size=9, color=C_PINK, bold=True)
        tb(slide, v, 1.88, y, 2.85, 0.28, font_size=9, color=C_DARK_TEXT)
        y += 0.28
    ...
    # ボーナス4種
    rect_b(slide, 0.2, 3.73, 9.6, 1.35, C_FILL_DEFAULT, border_color=C_CYAN, border_pt=1.2)
...
def slide3_normal(prs):
    ...
    # ─ 左：ポイント蓄積システム ─
    rect_b(slide, 0.15, 0.78, 4.6, 2.00, C_FILL_PINK, border_color=C_PINK, border_pt=1.5)
    ...
    for item in pt_items:
        tb(slide, f"・{item}", 0.3, y, 4.3, 0.28, font_size=8.5, color=C_DARK_TEXT)
        y += 0.30

    # 天井
    rect_b(slide, 0.15, 2.85, 4.6, 0.65, C_FILL_GOLD,
    ...
    tb(slide, "＊スルー回数が増えるほど初当たりAT期待値UP（スルー狙い有効）",
       0.3, 3.17, 4.3, 0.28, font_size=8, color=C_MID_GRAY)

    # ─ 右：CZ種別 ─
    rect_b(slide, 4.95, 0.78, 4.85, 1.90, C_FILL_CYAN, border_color=C_CYAN, border_pt=1.5)
    tb(slide, "CZ種別", 5.1, 0.82, 2.0, 0.32,
       font_size=10, color=C_CYAN, bold=True)

    czs = [
        ("マギアチャレンジ（通常）", "スイカ/ポイント消化で突入", C_DARK_TEXT),
    ...
    for cn, cd, cc in czs:
        ...
        tb(slide, f"   {cd}", 5.1, y + 0.24, 4.6, 0.22, font_size=8, color=C_MID_GRAY)
        y += 0.46

    rect_b(slide, 4.95, 2.77, 4.85, 1.40, C_FILL_PINK, border_color=C_PINK, border_pt=1.5)
    ...
    y = 3.17
    for mn, md in modes:
        tb(slide, f"・{mn}：", 5.1, y, 2.0, 0.28, font_size=9, color=C_PINK, bold=True)
        tb(slide, md, 7.05, y, 2.7, 0.28, font_size=9, color=C_DARK_TEXT)
        y += 0.34

    # ─ 下：打ち方メモ ─
    rect_b(slide, 0.15, 4.24, 9.65, 0.88, C_FILL_CYAN,
    ...
    my = 4.50
    for m in memo:
        tb(slide, f"・{m}", 0.3, my, 9.3, 0.21, font_size=8, color=C_MID_GRAY)
        my += 0.20
...
def slide4_cz(prs):
    ...
    # ─ マギアチャレンジ概要 ─
    rect_b(slide, 0.15, 0.78, 5.95, 3.1, C_FILL_PINK, border_color=C_PINK, border_pt=1.5)
    ...
    for k, v, vc in cz_info:
        tb(slide, f"◆ {k}", 0.3, y, 1.75, 0.28, font_size=9, color=C_PINK, bold=True)
        tb(slide, v, 2.0, y, 3.95, 0.28, font_size=9, color=vc)
        y += 0.35

    # 黒江チャレンジ
    rect_b(slide, 0.3, 2.45, 5.6, 0.88, C_FILL_GOLD,
    ...
    tb(slide, "マギアCZの本前兆中に昇格抽選あり。発生時点でAT当確レベルの強CZ",
       0.45, 2.82, 5.3, 0.28, font_size=8.5, color=C_DARK_TEXT)

    # CZ中の演出示唆
    rect_b(slide, 0.15, 3.40, 5.95, 0.72, C_FILL_CYAN, border_color=C_CYAN, border_pt=1)
    ...
    hx = 0.3
    for h in hints:
        tb(slide, f"・{h}", hx, 3.72, 2.7, 0.28, font_size=8.5, color=C_DARK_TEXT)
        hx += 2.0

    # ─ 右：前兆の見分け方 ─
    rect_b(slide, 6.25, 0.78, 3.6, 3.34, C_FILL_CYAN, border_color=C_CYAN, border_pt=1.5)
    tb(slide, "前兆の種類と見分け方",
       6.4, 0.82, 3.3, 0.35, font_name=FONT_H, font_size=11, color=C_CYAN, bold=True)

    zencho = [
        ("本前兆（弱）", "画面色が薄く変化\n小役連続で少し期待",   C_GRAY,  C_FILL_GRAY),
        ("本前兆（強）", "ステージ移行＋キャラ会話\nボーナス確率大幅UP", C_GOLD, C_FILL_GOLD),
        ("ガセ前兆",     "通常時にも発生\n演出で見分けが難しい",   C_GRAY,  C_FILL_GRAY),
        ("黒江前兆",     "固有演出で出現\nAT確定レベル",           C_GOLD,  C_FILL_GOLD),
    ]
    zy = 1.22
    for zt, zd, zc, zf in zencho:
        rect_b(slide, 6.35, zy, 3.4, 0.72, zf, border_color=zc, border_pt=1)
        tb(slide, zt, 6.5, zy + 0.05, 3.1, 0.28,
           font_size=9, color=zc, bold=True)
        tb(slide, zd, 6.5, zy + 0.30, 3.1, 0.36,
           font_size=8, color=C_DARK_TEXT)
        zy += 0.78
...
def slide5_at(prs):
    ...
    # AT概要バー
    rect_b(slide, 0.15, 0.78, 9.7, 0.6, C_FILL_PINK,
    ...
    for k, v in info_items:
        tb(slide, k, ix, 0.82, 1.2, 0.24, font_size=8, color=C_PINK, bold=True)
        tb(slide, v, ix, 1.06, 1.8, 0.24, font_size=9, color=C_DARK_TEXT)
        ix += 2.4

    # ─ 上乗せ手段（左）─
    rect_b(slide, 0.15, 1.50, 4.7, 2.72, C_FILL_GOLD, border_color=C_GOLD, border_pt=1.5)
    tb(slide, "上乗せ手段（出玉の伸ばし方）",
       0.3, 1.54, 4.3, 0.32, font_size=10, color=C_GOLD, bold=True)

    jouze = [
        ("レア役直乗せ",     "5〜100G\nレア役成立時に直接G数上乗せ",        C_DARK_TEXT),
    ...
    for jn, jd, jc in jouze:
        rect_b(slide, 0.25, jy, 4.5, 0.47, C_FILL_DEFAULT,
    ...
    # ─ ストーリーシステム（右）─
    rect_b(slide, 5.0, 1.50, 4.85, 2.72, C_FILL_CYAN, border_color=C_CYAN, border_pt=1.5)
    ...
    tb(slide, "AT中に8種のストーリーを収集し、コンプリートでエンディング→上位AT権利獲得",
       5.15, 1.90, 4.6, 0.42, font_size=8.5, color=C_DARK_TEXT)
    ...
    for i, s in enumerate(stories):
        col = C_GOLD if i == 7 else C_DARK_TEXT
        bg  = C_FILL_GOLD if i == 7 else C_FILL_DEFAULT
        bc  = C_GOLD if i == 7 else C_CYAN
        rect_b(slide, 5.1 + sx * 2.35, sy, 2.28, 0.30,
               bg, border_color=bc, border_pt=0.8)
...
def slide6_upper_at(prs):
    ...
    # 左：エンブリオイブ覚醒
    rect_b(slide, 0.15, 0.78, 5.75, 3.5, C_FILL_GOLD, border_color=C_GOLD, border_pt=2)
    ...
    tb(slide, "ストーリー8種コンプリートでエンディング到達後に突入する上位AT",
       0.3, 1.28, 5.5, 0.3, font_size=9, color=C_DARK_TEXT)
    ...
    # 右：ドッペルモード
    rect_b(slide, 6.05, 0.78, 3.72, 3.5, C_FILL_RED, border_color=C_RED, border_pt=2)
    ...
    for dk, dv in doppel_info:
        tb(slide, f"◆ {dk}：", 6.15, dy, 1.3, 0.3, font_size=9, color=C_PINK, bold=True)
        tb(slide, dv, 7.45, dy, 2.3, 0.3, font_size=8.5, color=C_DARK_TEXT)
        dy += 0.37

    # ドッペルモード演出ガイド
    rect_b(slide, 6.15, 3.05, 3.52, 0.95, C_FILL_RED,
           border_color=C_RED, border_pt=1)
    tb(slide, "ドッペルモード演出サイン",
       6.25, 3.09, 3.3, 0.28, font_size=9, color=C_RED, bold=True)
    tb(slide, "・画面が赤黒く染まる\n・魔女側のBGMに切り替わる\n・上乗せ数字が激増",
       6.25, 3.37, 3.4, 0.55, font_size=8, color=C_DARK_TEXT)
...
def slide7_design(prs):
    ...
    # アワード情報バナー
    rect_b(slide, 0.15, 0.78, 9.7, 0.46, C_FILL_GOLD,
    ...
    for i, (dt, dd, dc, tag, fill) in enumerate(designs):
        ...
        if i == 4:
            lx = 2.55
        rect_b(slide, lx, ly, 4.7, 1.05, fill, border_color=dc, border_pt=1.5)
...
def slide8_pros_cons(prs):
    ...
    # ─ 良い点（左）─
    rect_b(slide, 0.15, 0.78, 4.7, 4.15, C_FILL_GREEN, border_color=C_GREEN, border_pt=2)
    ...
    py = 1.28
    for pk, pv in pros:
        rect_b(slide, 0.25, py, 4.5, 0.52, C_FILL_GREEN,
               border_color=C_GREEN, border_pt=0.8)
        tb(slide, f"✓ {pk}", 0.38, py + 0.04, 1.7, 0.24,
           font_size=9, color=C_GREEN, bold=True)
        tb(slide, pv, 2.05, py + 0.04, 2.65, 0.44, font_size=8, color=C_DARK_TEXT)
        py += 0.60

    # ─ 課題（右）─
    rect_b(slide, 5.1, 0.78, 4.7, 4.15, C_FILL_RED, border_color=C_RED, border_pt=2)
    ...
    cy2 = 1.28
    for ck, cv in cons:
        rect_b(slide, 5.2, cy2, 4.5, 0.52, C_FILL_RED,
               border_color=C_RED, border_pt=0.8)
        tb(slide, f"✗ {ck}", 5.33, cy2 + 0.04, 1.7, 0.24,
           font_size=9, color=C_RED, bold=True)
        tb(slide, cv, 7.0, cy2 + 0.04, 2.65, 0.44, font_size=8, color=C_DARK_TEXT)
        cy2 += 0.60
...
def slide9_summary(prs):
    ...
    lessons = [
        ("学び①\nゴール設計", C_CYAN, C_FILL_CYAN,
         "コンプリートという「明確なゴール」を設けることで、AT中のプレイヤーの"
         "動機が持続する。ゴールが見えるゲームは打ち手を能動的にする。"),
        ("学び②\n逆転設計", C_RED, C_FILL_RED,
         "「穢れ」のように不利な状況を貯蓄して有利に転換する仕組みは、"
         "敗北体験を「投資感」に変換し、継続遊技へのモチベを生み出す。"),
        ("学び③\nIP×ゲーム性", C_GOLD, C_FILL_GOLD,
         "テーマ（魔法少女の絶望と穢れ）とゲームメカニクス（ドッペルモード）を"
         "直結させることで「演出に説得力」が生まれ、世界観浸透度が向上する。"),
    ]
    lx = 0.15
    for lt, lc, lf, ld in lessons:
        rect_b(slide, lx, 1.22, 3.15, 1.75, lf, border_color=lc, border_pt=2)
        tb(slide, lt, lx + 0.1, 1.26, 2.9, 0.45,
           font_name=FONT_H, font_size=12, color=lc, bold=True,
           align=PP_ALIGN.CENTER)
        tb(slide, ld, lx + 0.1, 1.70, 2.9, 1.20, font_size=8, color=C_DARK_TEXT)
        lx += 3.28

    # ─ 総括スコア（中段）─
    rect_b(slide, 0.15, 3.10, 9.7, 1.72, C_FILL_DEFAULT,
           border_color=C_GRAY, border_pt=0.8)
    tb(slide, "総括スコア（設計視点）",
       0.3, 3.14, 3.0, 0.3, font_size=10, color=C_MID_GRAY, bold=True)
    ...
    for sk, sv, sc in scores:
        tb(slide, sk, sx, 3.48, 1.75, 0.28, font_size=8.5, color=C_MID_GRAY, bold=True)
        tb(slide, sv, sx, 3.76, 1.75, 0.32, font_size=10, color=sc, bold=True)
        sx += 1.95

    # 総合評価
    rect_b(slide, 0.15, 4.44, 9.7, 0.55, C_FILL_GOLD,
           border_color=C_GOLD, border_pt=1.5)
    tb(slide,
