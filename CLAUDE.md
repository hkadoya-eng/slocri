...
4. 未収録のネタを10件選定してCSVを作成（手順0の除外パターンに該当するものは絶対に入れない・重視パターンの周辺ネタを優先）
   ...
   - **【品質ゲート 2026-05-28追加・必須】**
     ...
     7. **【飽和対策・最優先 2026-07-13】** info(機種情報)が約950件・総投稿1900件近く・495機種カバー済みで"量"は飽和。**単純な新台スペック収集はやめ切り口を変える**: (A)**【2026-09-01改定】**「読み物/横断」系＝機種間比較・ランキング・シリーズ変遷・メーカー特集・用語や業界の仕組み解説・気持ちよさ/演出考察(cat=column推奨・具体データ根拠つき)は**最大2件**（旧「最低3件」から上限に変更） (B)機種ネタは"同一機種でも打ち手目線の別角度"(天井/設定差/実戦/演出/気持ちよさ/打ち筋/やめ時、スペック羅列は不可) (C)名機エピソード(episode)の掘り起こし歓迎 (D)**【2026-09-01改定/新台主体】10件の内訳は new(導入前〜導入直後の新台)を最低3件・new+info を合計6件以上・column(読み物)は最大2件・episode(旧台)は最大1件・残りを jissen**（管理者指示「新台の投稿をメインに」。2026-08-25改定の「new/info最低4件」でも直近120件は column 33%・new+info 26%で読み物が主役のままだったため件数の主役を新台に戻した。中身の(B)(E)は緩めない）。(E)**【2026-08-25追加】打ち手が既に知っている内容・誰向けか分からない読み物・タイトルだけで意味が通らないものは投稿しない。**※neta_0900/1330.txt と同期済み・詳細は memory feedback_neta_saturation_pivot
