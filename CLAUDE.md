...
4. 未収録のネタを10件選定してCSVを作成（手順0の除外パターンに該当するものは絶対に入れない・重視パターンの周辺ネタを優先）
   ...
   - **【品質ゲート 2026-05-28追加・必須】**
     ...
     7. **【定番と独自のバランス・2026-09-10改定/最優先】どちらかに寄せない。両方に下限を置く。**
        ・**(あ) 巷の話題（定番ネタ）最低4件**＝複数のまとめ/解析サイトが今まさに扱っている話題。**みんながアンテナを張っていて間違いのない情報なので必ず押さえる**。目新しさは不要で後追いでよい（2026-09-10に最低3→4件へ増やした）
        ・**【SIS集計の落とし穴・2026-09-14追加/必読】稼働値を期間で比べるときは必ず曜日を揃える（同曜日同士か7日平均）。全国平均アウトは土日が平日の1.50倍**（105日実測）なので、火曜と日曜を比べるだけで分母が約47%膨らみ機械的に約3割落ちて見える。2026-09-14にこれで誤投稿を出した（実際は1台あたり生アウト-4.3%〜+3.5%の横ばい・台数は29〜34%増）。書く前に①同曜日/7日平均で比べたか②生アウトでも同じ向きか③台数はどう動いたか、の3点を確認する。詳細は memory bug_katsudo_weekday_effect
        ・**(い) 独自観点1〜2件**＝自前SIS集計・機種横断比較・他サイトが出していない切り口。**0件にしない**（スロキーだけが出せる価値）が、新台と巷の話題に枠を回すため2〜3→1〜2件へ縮小。**column に載せる必要はなく、新台の初日データのように new/jissen に載せてよい**（新台主体の配分とはこの形で両立させる）
        ・(う) 残り4〜5件は通常の機種ネタ（解析・実戦・演出・打ち筋）を外部記事ベースで
        ・**同一ドメインは最大4件・全体で最低4ドメイン**。1サイト丸写し状態でなければよく、無理に珍しいサイトを探さない
        ・source列にサイト名を必ず書く（複数参照は「なな徹 / ちょんぼりすた」と連記）・url列は「OGP画像が取れるソース」を優先。(い)は source に「自社SIS全国日次」等と書き url は空でよい
        ・**他サイトの後追いは歓迎**＝話題選び・切り口は真似てよく、**解析値/スペック/実戦値/口コミの数字はそのまま使ってよい**（事実に権利は無い）。ただし**本文はコピペせず自分の言葉で書き直し**、出典サイト名とURLを必ず付ける
        ・(E)の「数字を見ないと分からないこと」の"数字"は**自前SIS集計に限らない**（他サイトの解析値・実戦値・口コミ集計も同じ資格）。自前SIS集計は強みなので使ってよいが、それだけで10件を埋めない
        経緯: 直近80件で url無し22件(28%)・ちょんぼりすた22件と偏り、管理者指摘「分析によりすぎている／他のサイトをくまなく調査して」（2026-09-10）。
        **続けて「まとめサイトの情報はみんながアンテナをはっていて間違いがないのである程度は押さえる＋独自の観点も入れる、というバランスの話。極論にならないように」との指示があり、「自前分析は最大2件・最低5ドメイン」という抑制一辺倒だった初版を両方に下限を置く形へ改めた。片方を潰す方向に振らないこと。**
     8. **【本文とタイトルの文体・2026-09-10追加/全catに適用】打ち手目線の口語で書く。論文・レポート調にしない。**
        管理者指摘「書き方がちょっと論文ぽいので、もうちょっとくだけた感じにできますか？サイトの口調や観点に近い感じ」（2026-09-10）。**煽らないルール（2026-08-21）と両立する＝くだける≠煽る/ノリで書く。**
        ・**主語を打ち手側に置く**（×「打ち手からは〜という書き込みが並んだ」→ ○「打ってる側からすれば『せっかくAT引いたのに』で終わる」）
        ・**スペックの括弧列挙を禁止**（×「（サミー・機械割97.9〜114.6%・AT初当り1/328.8〜1/256.7・純増約8.4枚・ベース約32G）」）。必要な数字だけ文中に置き**1文に数字は1〜2個まで**
        ・**体言止め・名詞句の連結を避ける**（×「〜という構成。」「〜の水準にある。」→ ○「〜になっている」）・**1文50字程度で短く刻む**
        ・**話し言葉の接続でつなぐ**（〜だけど/だから/実際/そのぶん/だけに）。論説の接続（一方で/つまり/〜に集中した/同居している）を並べない
        ・口コミ引用は活かし**引用の後を打ち手の実感で受ける**・内輪指標（1台1日アウト・稼働値・コイン単価）は使うならその場で1回だけ意味を書く
        ・**タイトルも同じ調子で短くくだす**（×「リコリコの初打ちの不満は上位CZに失敗すると下位ATまで終わる仕様に集まった」→ ○「リコリコの不満は上位CZを落とすとATごと終わるところに集中」）。言い切ること・一覧で中身が分かることは崩さない
        ・**やらないこと**: 「www」「草」「〜〜！！」のノリに振る／口調を変えるために数字・出典・事実を落とす／煽り語と「──」「——」（従来通り禁止）
        ※反映先は neta_0900/1330・req_tick・column_weekly・idea_column の5プロンプト。詳細は memory feedback_writing_no_hype
     9. **【飽和対策・最優先 2026-07-13】** info(機種情報)が約950件・総投稿1900件近く・495機種カバー済みで"量"は飽和。**単純な新台スペック収集はやめ切り口を変える**: (A)**【2026-09-10改定】**「読み物/横断」系＝機種間比較・ランキング・シリーズ変遷・メーカー特集・用語や業界の仕組み解説・気持ちよさ/演出考察(cat=column推奨・具体データ根拠つき)は**最大1件**（2026-09-01の最大2件からさらに縮小・旧「最低3件」から上限に変更） (B)機種ネタは"同一機種でも打ち手目線の別角度"(天井/設定差/実戦/演出/気持ちよさ/打ち筋/やめ時、スペック羅列は不可) (C)名機エピソード(episode)の掘り起こし歓迎 (D)**【2026-09-10改定/新台と巷の話題をさらに増やす】10件の内訳は new(導入前〜導入直後の新台)を最低4件・new+info を合計7件以上・column(読み物)は最大1件・episode(旧台)は最大1件(0でよい)・残りを jissen。標準形は new4+info3+jissen2+column1**（管理者指示「基本新台メインの投稿にしてみる？やっぱり新台や巷の話題がもう少し多い方が良いかな」2026-09-10。2026-09-01改定で new33%・new+info62%・column7%まで戻っていたところを、もう一段新台寄りにした。**"もう少し"の調整なので new を6〜7件にするような極端化はしない**。中身の(B)(E)は緩めない＝新台でもスペック羅列は不可）。(E)**【2026-08-25追加】打ち手が既に知っている内容・誰向けか分からない読み物・タイトルだけで意味が通らないものは投稿しない。**※neta_0900/1330.txt と同期済み・詳細は memory feedback_neta_saturation_pivot
...
**2026-08-03追加: 手順10「SIS実績の同期」** = `python scripts/misc/update_sis_record.py` を実行し、機種ごとの `sisRecord`（稼働貢献◯週で終了/継続中・設置週数・**死に台週数**＝設置−貢献・直近稼働値）を machineAnalysis へ書き込む。**新台診断ビューは直近約26週の機種しか表示しないため、窓を出た機種の「◯週で終了」がどこにも残らなかった**のを補うステップ。機種分析シートに常設表示され、過去機種の答え合わせ(FB)を後から参照できる。終了判定は「直近4週すべて稼働値100%以下、または撤去」＝貢献週がもう増えない状態（設置終了を条件にすると死に台が数十週放置されるため誤判定になる）。冪等なので差分0なら何もしない。9:00/13:30の両プロンプトに記載済み。
...
**2026-08-21追加: 演出・表現分析を2軸→4軸に拡張。** `presentationDesign.表現の型` に **昇格の型**（上の状態へどう上がるか＝突破/ストック/到達/段差/上乗せ/抽選/状態経由/格差）と **評判になっている表現**（machineAnalysis の pros/cons から演出・音・映像・出目・筐体に触れた声を型に分けたもの。称賛13型/批判15型・82機種）を追加。この2軸は `python scripts/misc/build_expression_axes.py` が生成する（冪等）ので、**週次ジョブの手順4で毎回実行し、型名や説明を json 側で手編集しないこと**（次の実行で上書きされる）。判定語を足したいときはスクリプトの C_RULES / D_PRAISE / D_BLAME に追記する。

**2026-08-03追加: 表現分析(presentation)の付与をフォーカス(e)として追加。** machines[].presentation に intervention(介入度0〜3)/interventionNote/**interventionScope**/**interventionPayoff**/scopeNote/noticeType/disclosure/soundDesign/cabinet/escalation/scoredAt/scoredBasis/evidence を入れる。判定基準は `presentationDesign.介入度スコア基準` に定義済み（intervention: 0=完全自動 / 1=押し順のみ・周期やポイント到達型 / 2=狙い目がある / 3=常設の自力契機。scope: 常時/状態中/局所/なし＝どこで介入できるか。payoff: 出玉量に直結/当否のみ/なし＝介入が何に効くか）。**突破契機は実機解析で裏取りしURLを evidence に必須記載**。1回の実行で最大3機種。
> **⚠ interventionの0〜3の定義は変更しない。** 粒度不足を感じても intervention をいじらず、観察できる事実を別フィールドとして足す。寿命に合わせて基準を後から調整すると過剰適合になる（scope/payoffを足したのはまさにこの方針に沿った対応で、パイロットで介入度2が真逆に割れた原因が「基準の粗さ」ではなく「範囲と効く先を記録していなかったこと」だったため）。
> **⚠ この値を新台診断の仕分けに使ってはいけない（現状は説明軸のみ）。** 理由: 貢献週が確定した126機種のうちgameDesignLibraryと突合できるのは25件で母数不足、かつ2週の数値だけでほぼ分離できており残差が2件しかない。さらにパイロットで介入度2の機種が真逆に割れた（東京喰種=76週 vs ダンバイン=8週・麻雀物語=7週）＝粗い0〜3では寿命を分離できないことが判明済み。**予測軸への昇格条件（確定台×presentation付与が40件超で既存2軸の相関を上回ること）を先に固定してある**ので、後から都合よく閾値を探さない。詳細は `presentationDesign.介入度の検証状況`。
