...
**2026-08-03追加: 表現分析(presentation)の付与をフォーカス(e)として追加。** machines[].presentation に intervention(介入度0〜3)/interventionNote/noticeType/disclosure/soundDesign/cabinet/escalation/scoredAt/scoredBasis/evidence を入れる。判定基準は `presentationDesign.介入度スコア基準` に定義済み（0=完全自動 / 1=押し順のみ・周期やポイント到達型 / 2=狙い目がある / 3=常設の自力契機）。**突破契機は実機解析で裏取りしURLを evidence に必須記載**。1回の実行で最大3機種。
