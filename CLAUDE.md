...
3. **【サイト巡回・2026-09-10全面改定】WebSearchだけで済ませない。sources_and_rules.md の27サイトから最低8サイトを WebFetch で実際に開いて回る**（手順0の重視/除外パターンを意識する）
   - うち2サイト以上は手順0で「直近40件に0件」だったサイトから選ぶ（毎回ちょんぼりすた/DMMぱちタウンに戻らないため）
...
4. 未収録のネタを10件選定してCSVを作成（手順0の除外パターンに該当するものは絶対に入れない・重視パターンの周辺ネタを優先）
   ...
   - **【品質ゲート 2026-05-28追加・必須】**
     ...
     7. **【出典ゲート・2026-09-10追加/最優先】10件の出典を散らす。満たさないCSVは作り直す。**
        ・**url無し（自前SISデータの独自集計だけで書いた投稿）は最大2件**。残り8件は必ず外部記事のURLを持つ
        ・**同一ドメインは最大3件**・**全体で最低5ドメイン**にまたがる
        ・source列にサイト名を必ず書く（複数参照は「なな徹 / ちょんぼりすた」と連記）・url列は「OGP画像が取れるソース」を優先
        ・**他サイトの後追いは歓迎**＝話題選び・切り口は真似てよく、**解析値/スペック/実戦値/口コミの数字はそのまま使ってよい**（事実に権利は無い）。ただし**本文はコピペせず自分の言葉で書き直し**、出典サイト名とURLを必ず付ける（丸写しはサイトの信用と検索評価を落とす）
        ・(E)の「数字を見ないと分からないこと」の"数字"は**自前SIS集計に限らない**。他サイトの解析値・実戦値・口コミ集計を主に使う＝**自前SIS集計に寄せるのは禁止**
        経緯: 直近80件で url無し22件(28%)・ちょんぼりすた22件・DMMぱちタウン10件と偏り、管理者指摘「投稿の内容が分析によりすぎている／他のサイトをくまなく調査して」（2026-09-10）
