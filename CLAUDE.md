...
4. 未収録のネタを10件選定してCSVを作成（手順0の除外パターンに該当するものは絶対に入れない・重視パターンの周辺ネタを優先）
   ...
   - **【品質ゲート 2026-05-28追加・必須】**
     ...
     7. **【飽和対策・最優先 2026-07-13】** info(機種情報)が約950件・総投稿1900件近く・495機種カバー済みで"量"は飽和。**単純な新台スペック収集はやめ切り口を変える**: (A)10件中 最低3件は「読み物/横断」系＝機種間比較・ランキング・シリーズ変遷・メーカー特集・用語や業界の仕組み解説・気持ちよさ/演出考察(cat=column推奨・具体データ根拠つき) (B)機種ネタは"同一機種でも打ち手目線の別角度"(天井/設定差/実戦/演出/気持ちよさ/打ち筋/やめ時、スペック羅列は不可) (C)名機エピソード(episode)の掘り起こし歓迎 (D)「新台情報」の単純追加はnew/info合計で最大2件まで。※neta_0900/1330.txt と同期済み・詳細は memory feedback_neta_saturation_pivot
